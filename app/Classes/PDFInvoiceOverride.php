...
class PDFInvoiceOverride extends Invoice
{
    ...
    public function render()
    {
        ...
        $this->pdf = PDF::loadView('vendor.invoices.templates.emara', ['invoice' => $invoice_data->invoice, 'invoice_items' => $invoice_data->invoice_items]);
        $output = $this->pdf->output();

        return Storage::disk('invoices')->put(settings()->get('sfc_serial') . $invoice_data->invoice->invoice_number . '.pdf', $output);
    }
